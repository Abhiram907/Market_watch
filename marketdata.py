...
class StreamlitUI:
    def __init__(self):
        ...
        self.last_update_time = {}  # Track last update time for each row
    ...
    def style_dataframe(self, df):
        """Style the dataframe with colors and number formatting"""
        def color_pnl(val):
            try:
                # Fix for single element Series deprecation warning
                if isinstance(val, pd.Series):
                    val = val.iloc[0]
                val = float(val)
                return ['color: red' if val < 0 else 'color: green' if val > 0 else ''] * len(df.columns)
            except:
        ...
        return df.style.format({
            ...
            'SL': '{:.2f}',
            'STRIKE': '{:.2f}'
        }).apply(color_pnl, axis=1)  # Add axis=1 to apply row-wise

    def display_data(self):
        """Display the data using a persistent container"""
        # Create container only once and store in session state
        if 'data_container' not in st.session_state:
            st.session_state.data_container = st.empty()
        
        # Update existing container without creating new elements
        with st.session_state.data_container:
            styled_df = self.style_dataframe(st.session_state.data)
            st.dataframe(
                styled_df,
    ...
    def update_market_data(self):
        ...
        # Process rows with staggered updates
        for idx, row in st.session_state.data.iterrows():
            # Update row if it hasn't been updated in the last second
            if idx not in self.last_update_time or (current_time - self.last_update_time[idx]) >= 1:
                updated_data = self.process_row(row, idx)
                if updated_data:
                    updates.append((idx, updated_data))
                    self.last_update_time[idx] = current_time
        
        # Apply all updates at once
        if updates:
            for idx, update_data in updates:
